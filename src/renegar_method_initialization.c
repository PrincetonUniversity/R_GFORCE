...
void add_random_shuffle_par(const int d,const int num_shuffles, double* const restrict E,
    ...
    const int d2 = d*d;
    precompute_all_shuffles_par(d,num_shuffles,all_shuffles);
...
void precompute_all_shuffles_par(const int d, const int num_shuffles, int* restrict shuffles){
    ...
    #pragma omp for
    for(int i=0; i < num_shuffles; i++){
        int* const restrict shuffle_row = shuffles + i*d;
        random_shuffle_threadsafe(d,shuffle_row,this_thread_rng);
    }
